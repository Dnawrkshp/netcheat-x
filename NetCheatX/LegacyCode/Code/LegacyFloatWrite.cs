...
{
    public class LegacyFloatWrite : ILegacyType
    {
        private ulong _address = 0;
        private byte[] _value = null;

        public char Type { get; } = '2';
        ...
        public bool FromString(IPluginHost host, ref int index, string[] lines)
        {
            ...
            try
            {
                ...
                if (words[0].Length > 1 && words[0][1] == '1')
                    _value = host.PlatformBitConverter.GetBytes(double.Parse(words[2]));
                else
                    _value = host.PlatformBitConverter.GetBytes(float.Parse(words[2]));
            }
            catch { return false; }
